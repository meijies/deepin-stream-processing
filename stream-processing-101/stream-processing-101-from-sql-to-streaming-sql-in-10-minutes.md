...
我们进入了一个分析，理解和响应组织数据成为竞争优势的时代。当这样做时，时间是至关重要的，速度将决定输赢。

现实情况是，大多数数据的价值随着时间而下降。有趣的是，昨天发生了交通堵塞、10起欺诈事件或10人心脏病发作。根据这些历史数据，我们可以学习如何减轻或预防未来的这些事件。但是，如果我们能够在它们发生的时候了解这些状况，那么我们就可以进行干预和管理。

我们中的许多人都经历过及时的数据如何帮助我们的生活。假设您正在沿着这条路行驶，突然间，引擎故障灯亮起来。当然，这很烦人，但是获得早期警告非常有用 - 通常在车辆偏离正常行为或遇到众所周知的故障模式时触发，例如当电池的充电充电线路故障。幸运的话，你可以转到最近的车库，修好它，然后继续前行而不是被困在路上。

汽车制造商已经考虑过这里描述的车辆情景。同样，一些组织正在研究某些用例（例如股票市场，患者监控和监控）可以通过即时访问数据以便及时采取纠正措施并获益。但是，还有许多其他用例尚未经过检查。

多年来，我们尝试使用数据库和批处理技术（如Hadoop和Spark）来分析数据并快速响应以改善结果。这两种技术都非常适合于许多应用，但它们不是快速响应数据工具的最佳选择。相反，旨在处理实时场景的技术称为流处理或流分析。

另一方面，被设计成一次数据传递的流并不能适用于所有场景。对于可以通过单次传递数据进行处理或具有时间局部性（处理倾向于访问最近数据）的用例，流式传输非常有用。相比之下，训练机器学习算法和图形处理都需要多次通过数据集，因此流处理不适合。
...
现在你需要对数据做些什么。执行此操作的旧方法是编写将处理与事件源连接起来的代码，然后逐个处理每个事件。事件处理体系结构中的一个有用单元是“actor”或“agent”，它接受事件并产生另一个事件。这actor在概念上将一个流转换为另一个流。
![streaming-sql-1](introduction-to-streaming-analytics-1.png)

为了理解这些想法，让我们考虑一个带有温度传感器的锅炉，例如，假设我们想要检测来自锅炉的温度流何时大于350°C。另一个actor可以使用这个新流来创建另一个流。实际上，actor可能会更复杂，接受多个流以产生复合流。当您使用actor连接多个流时，最终会得到一个处理图。事件然后流经图并产生新的事件。

假设我们在锅炉内部和锅炉外的温度流上计算“30秒运行平均值”，并且我们希望在两个温相差太大时产生一个事件。计算此查询的图表如下所示。
![streaming-sql-2](introduction-to-streaming-analytics-2.png)

这里前两个Actor需要记住有关早期事件的数据（例如总和和计数）。如果系统出现故障，它将丢失该数据。因此，我们需要构建一种保存和恢复这些状态的方法。
